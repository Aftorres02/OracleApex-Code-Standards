...
- **Between procedures and functions**: after `gc_scope_prefix` and after each `end <unit>;`, insert **exactly 8 blank lines** before the next unit's JavaDoc or declaration. This is the single authoritative number — do not use 6 or 1 (see [`package_body_template.sql`](../templates/package_body_template.sql), which has been corrected to match).
...
Use the ready-to-copy skeletons in [`package_spec_template.sql`](../templates/package_spec_template.sql) and [`package_body_template.sql`](../templates/package_body_template.sql). They already encode §4 (8-blank-line spacing), §6 (parameter alignment), §9–11 (documentation tags) and §8 (logging).
