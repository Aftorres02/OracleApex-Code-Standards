...
| ---------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| [`all_apex.sql`](all_apex.sql)           | Yes            | Will install all the APEX applications                                                                                              |
...
| [`load_env_vars.sql`](load_env_vars.sql) | Yes            | Loads the environment variables into the SQL session                                                                                |
...
Oracle releases are very "unforgiving" (i.e. if an error occurs it can ruin the entire rest of release and make it difficult to back out). This can make it very difficult to run standard CI/CD tools to dor releases in Oracle as one error can completely corrupt an environment and it may not be easy to "restore" the schema to a point in time before the release. _Note: Making releases scripts re-runnable or providing roll back support is not impossible however it can be very expensive to do so properly._,
...
_Note: [semantic versioning](https://semver.org/) is recommended for version numbers. TL;DR: `major.minor.patch` numbering._
...
**Important note:** It is critical that when modifying a file with a DDL the DDL is put in the correct order it _should_ have been run.
...
# For example if the project's base folder is in /users/developer/git/my-apex-project then the call would be: reset_release my-apex-project
...
- Using [Visual Studio Code](https://code.visualstudio.com/) (VSC) (or similar text editor)
  ...
  - Shortcuts setup to quickly navigate between current file and terminal. In VSC `Preferences > Keyboard Shortcuts`: (_Windows users change `cmd` to `ctrl`_)
